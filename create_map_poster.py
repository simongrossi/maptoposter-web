...
THEMES_DIR = "themes"
FONTS_DIR = "fonts"
POSTERS_DIR = "static/posters"  # Modified for SvelteKit integration
...
def load_fonts():
    ...
    for weight, path in fonts.items():
        if not os.path.exists(path):
            # print(f"⚠ Font not found: {path} (using system fallback)")
            return None
...
def create_poster(city, country, point, dist, output_file, output_format, width=12, height=16, country_label=None, name_label=None):
    ...
    with tqdm(total=3, desc="Fetching map data", unit="step", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt}') as pbar:
        ...
        # 3. Fetch Parks Feature
        pbar.set_description("Downloading parks features")
